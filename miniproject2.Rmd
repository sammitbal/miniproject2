...
**Our Group's Question:** "How is the score related to the result (win/loss)?" 
...
**Step 1: Calculate `logit`:** Calculate logit form (e.g. $\beta_0 + \beta_1 x_{i,1}$)

$\ -3.324e-01 + \ 1.689e-04 x_{i,Score}$

**Step 2: Convert `logit` to `odds`:** Take $e$ to this result (i.e. $e^{\beta_0 + \beta_1 x_{i,1}} = e^{-????} \approx ????$)


**Step 3: Interpret the Slope:** As a person’s [insert VAR] increases by 1 [insert UNIT], we expect the odds of [insert SUCCESS] to change by a factor of [insert FACTOR (ODDS)].

- Note that the explanation reads “change by” rather than “increase by” or “decrease by”

*Example Interpretation:* (In class)

"As a person's age increases by 1 year..." 

 - We expect the odds of surviving to change by <u>a factor of</u> 0.991. 
 - We expect the <u>odds of surviving to decrease by</u> 0.9%. 

**Step 4 (You can merge this into step 3 if you want): Answer the Model Using the Results:** Put the model into context!

1. Plug in the min/max, averages, and other statistics into the $\beta_0$ and $\beta_1$s in the equation, and
2. Interpret the results. Can you answer the question, even?

- NOTE: Part of your grade for this question will be determined based on whether your question is a valid question about inference and whether your analysis allows you to answer the question.

- (my note): ...meaning you should note & provide explanation if the results DON'T allow you to answer the question.




```{r}
# A glimpse at Score (USED FOR STEP 4)
COD_dataset %>%
  select(Score) %>%
...
```{r}
exp(-3.324e-01 + (1.689e-04 *100)) # odds for min 
exp(-3.324e-01 + (1.689e-04 *7315)) #odds for max
exp(-3.324e-01 + (1.689e-04 *2944.531)) # odds for mean
exp(-3.324e-01 + (1.689e-04 *2770	)) #odds for median
exp(-3.324e-01 + (1.689e-04 *1464.965)) #odds for sd
```

As a teams minimum score increases by 1 point, we expect the odds of winning to change by a factor of 0.7294168.

As a teams maximum score increases by 1 point, we expect the odds of winning to change by a factor of 2.467248.

As a teams average score increases by 1 point, we expect the odds of winning to change by a factor of 1.179312.

As a teams median score increases by 1 point, we expect the odds of winning to change by a factor of 1.145055.

As a teams standard deviation score increases by 1 point, we expect the odds of winning to change by a factor of 0.9185422
