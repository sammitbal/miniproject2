...
# Greating generalized linear regresison model
...
- *Note:* In spite of the `Pr(>|z|)` of the Score and Intercept exceeding the $\alpha$ value of $0.05$, <u>we still will use these figures</u>, as an $\alpha$ value of $0.05$ is inappropriate for predicting categorical variables, and specifically for slope, an $\alpha$ value of $0.15$ or greater would be appropriate, of which this probability exceeds (is below).  
...
**Interpretation of the Probabilities:** Based on the table above--- which measures the distributions of winning by aggregating the probabilities of winning of each Score--- we can see that the majority of these summary statistics, starting from the 1st quartile, exceed a 50% probability of winning the game. Basically, Players in this data set's win rate is pretty high, regardless of their score. 

And while this is an interesting observation, this does not answer the question. Rather, the difference between the `min()` Score's probability of winning versus the `Q1`, `mean`/`median`, `Q3`, and `max` indicates an <u>increase in the chance of winning</u> as the score of the Player in this game rises (in comparison to the Scores in other games in the data set). Combine that with the calculations in Step 2, which suggest  And therefore, with reasonable certainty, we can answer the question: 
